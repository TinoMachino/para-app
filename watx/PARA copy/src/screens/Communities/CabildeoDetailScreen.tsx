...
export function CabildeoDetailScreen({route}: Props) {
  ...
  const [delegateOverrideOption, setDelegateOverrideOption] = useState<
    number | null
  >(null) // the chosen option during override
  ...
  const gracePeriodRemainingMs = useMemo(() => {
    ...
    return Math.max(0, expiryTime - now.getTime())
  }, [delegateEvent, now])
        ...
        <Layout.Center style={styles.center}>
          ...
          <View style={styles.positionList}>
            {positions.map((pos, i) => {
              ...
              return (
                <View key={i} style={[styles.positionCard, t.atoms.bg_contrast_25, isLowQuality && {opacity: 0.5}]}>
                  <View style={styles.positionHeader}>
                    ...
                    {pos.optionIndex !== undefined && (
                      <Text style={[styles.posOptionRef, t.atoms.text_contrast_medium]}>
                    ...
                    <View
                      style={[
                        ...
                        isHighQuality ? {backgroundColor: '#34C759' + '20', borderColor: '#34C759' + '40'} :
                        isLowQuality ? {backgroundColor: '#FF3B30' + '20', borderColor: '#FF3B30' + '40'} :
                        {backgroundColor: t.palette.contrast_50, borderColor: t.palette.contrast_200}
                      ]}>
                      <Text
                        style={[
                          styles.constructivenessBadgeText,
                          isHighQuality ? {color: '#34C759'} :
                          isLowQuality ? {color: '#FF3B30'} :
                          t.atoms.text_contrast_medium,
                        ]}>
                        {isHighQuality ? '✨' : isLowQuality ? '⚠️' : '📊'} {Math.round(score * 100)}%
                      </Text>
                    </View>
                  </View>
                  
                  <Text style={[styles.positionText, t.atoms.text]}>
                    {isLowQuality ? 'Este comentario ha sido marcado como poco constructivo por la comunidad.' : pos.text}
                  </Text>
                  
                  {isLowQuality && (
                    <Text style={[styles.lowQualityReason, {color: '#FF3B30'}]}>
                      Score de constructividad demasiado bajo para ser incluido en la síntesis IA.
                    </Text>
                  )}

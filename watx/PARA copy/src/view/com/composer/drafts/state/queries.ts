...
export async function loadDraftMedia(draft: AppBskyDraftDefs.Draft): Promise<{
  ...
  if (draft.deviceId && draft.deviceId !== getDeviceName()) {
    return { loadedMedia }
  }
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (existingDraftId) {
        ...
        // Add deviceId to new drafts
        const draftWithDevice = {
          ...draft,
          deviceId: getDeviceName(),
        }
        const res = await agent.app.bsky.draft.createDraft({
          draft: draftWithDevice,
        })
        draftId = res.data.id
    ...
    onSuccess: async (
      { draftId, localRefPaths, originalLocalRefs },
      { composerState },
    ) => {
      ...
      await saveDraftMetadata(draftId, composerState.thread.posts)
...
export function useDeleteDraftMutation() {
  ...
  return useMutation({
    ...
    onSuccess: async (_, { draftId, draft }) => {
      ...
      await deleteDraftMetadata(draftId)
...
export function useCleanupPublishedDraftMutation() {
  ...
  return useMutation({
    ...
    onSuccess: async (_, { draftId, originalLocalRefs }) => {

...
interface PostMetaOpts {
  ...
  /** Post flair displayed at the end of the meta row */
  postFlair?: {
    label: string
    color: string
    bgColor: string
...
let PostMeta = (opts: PostMetaOpts): React.ReactNode => {
  ...
  const postFlairs = opts.postFlairs || (opts.postFlair ? [opts.postFlair] : [])

  return (
    <View
      style={[
        // a.flex_1, // This was causing vertical alignment issues in FeedItem
        a.flex_row,
        a.align_center,
        a.pb_xs,
        a.gap_xs,
      ...
      {/* Post flair badge - like "Agenda Post" */}
      {postFlairs.map((flair, index) => (
        <View
          key={`${flair.label}-${index}`}
          style={[
            a.px_sm,
            a.py_xs,
            a.rounded_sm,
            a.ml_xs,
            {backgroundColor: flair.bgColor},
          ]}>
          <Text style={[a.text_xs, a.font_bold, {color: flair.color}]}>
            {flair.label}
          </Text>
        </View>
      ))}

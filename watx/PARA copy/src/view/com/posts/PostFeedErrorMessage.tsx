...
function FeedgenErrorMessage({
    ...
    () =>
      ({
        ...
        [KnownError.FeedSignedInOnly]: _l(
          msgLingui`This content is not viewable without a PARA account.`,
        ),
        ...
        [KnownError.IdentityInvalid]: _l(
          msgLingui`Your session appears to be invalid. Please sign out and sign in again.`,
        ),
      })[knownError],
    [_l, knownError],
  )
  ...
  const cta = React.useMemo(() => {
    switch (knownError) {
      ...
      case KnownError.IdentityInvalid: {
        return (
          <Button
            type="primary"
            label={_l(msgLingui`Sign Out`)}

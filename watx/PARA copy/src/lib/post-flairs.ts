import {parseTag, POST_FLAIRS, POST_TYPES, type PostFlair, type PostType} from '#/lib/tags'
...
export function normalizeComposerFlairs(flairs: ComposerFlair[]): ComposerFlair[] {
...
function resolveBadgeFromTag(
  ...
  const color =
    matched?.color || (kind === 'policy' ? DEFAULT_POLICY_COLOR : DEFAULT_MATTER_COLOR)
...
export function getPostBadges(record: PostBadgeRecord): PostBadge[] {
  ...
  if (badges.length === 0 && fallbackKind) {
    ...
    const parsed = fallbackTag ? parseTag(fallbackTag) : {isOfficial: false, type: null}
    const label = `${parsed.isOfficial ? 'Official' : 'Community'} ${
      fallbackKind === 'policy' ? 'Policy' : 'Matter'
    }`
    const color = fallbackKind === 'policy' ? DEFAULT_POLICY_COLOR : DEFAULT_MATTER_COLOR
    badges.push({

...
describe('crud operations', () => {
  ...
  it('enforces single com.para.status record key per user', async () => {
    const now = new Date().toISOString()
    const created = await bobAgent.api.com.atproto.repo.createRecord({
      repo: bobAgent.accountDid,
    ...
    expect(new AtUri(created.data.uri).rkey).toBe('self')

    await expect(
      bobAgent.api.com.atproto.repo.createRecord({
        repo: bobAgent.accountDid,
        collection: ids.ComParaStatus,
        rkey: TID.nextStr(),
        record: {
    ...
    ).rejects.toThrow('Invalid com.para.status record key: must be "self"')
  })

  it('enforces com.para.social.postMeta to target local com.para.post using matching rkey', async () => {
    const now = new Date().toISOString()
    const createdPost = await aliceAgent.api.com.atproto.repo.createRecord({
      repo: aliceAgent.accountDid,
    ...
    const postUri = new AtUri(createdPost.data.uri)

    const createdMeta = await aliceAgent.api.com.atproto.repo.createRecord({
      repo: aliceAgent.accountDid,
    ...
    expect(new AtUri(createdMeta.data.uri).rkey).toBe(postUri.rkey)
  ...
  it('rejects com.para.social.postMeta that references non-local or non-para posts', async () => {
    const now = new Date().toISOString()
    const bskyPost = await aliceAgent.api.com.atproto.repo.createRecord({
      repo: aliceAgent.accountDid,
    ...
    const bskyUri = new AtUri(bskyPost.data.uri)
    await aliceAgent.api.com.atproto.repo.deleteRecord({
      repo: bskyUri.host,
      collection: bskyUri.collection,
      rkey: bskyUri.rkey,
    })

    const bobParaPost = await bobAgent.api.com.atproto.repo.createRecord({
      repo: bobAgent.accountDid,
      collection: ids.ComParaPost,
      record: {
        text: 'bob para',

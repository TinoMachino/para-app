...
import {SquareBehindSquare4_Stroke2_Corner0_Rounded as _DeckIcon} from '#/components/icons/SquareBehindSquare4'
...
export function MemesAndDocumentsScreen({
  ...
  const [_focusedItemId, setFocusedItemId] = useState<string | undefined>()
  ...
  }, [documents, query, activeFilters])

  const _activeItems =
    activeMode === 'Memes' ? filteredMemes : filteredDocuments
  const boardWidth = width > 900 ? (width - 52) / 2 : undefined

  const _setNextMode = (mode: Mode) => {
    setActiveMode(mode)
    setFocusedItemId(undefined)
    navigation.setParams({mode, view: viewStyle})
  }

  const _setNextView = (next: ViewStyleMode) => {
    setViewStyle(next)
  ...
  return (
    <Layout.Screen testID="memesAndDocumentsScreen">
      ...
      <Layout.Content
        ...
        {/* ── Memes Section ── */}
        <View style={[styles.sectionCard, t.atoms.bg_contrast_50]}>
          ...
          {filteredMemes.length === 0 ? (
            <Text style={[styles.sectionEmpty, t.atoms.text_contrast_medium]}>
              {_(msg`No hay memes con esos filtros`)}
            </Text>
          ) : (
            <View style={styles.boardGrid}>
              {filteredMemes.slice(0, 6).map(item => (
                <MediaCard
                  key={item.id}
                  item={item}
                  mode="Memes"
          ...
          {filteredMemes.length > 6 && (
            <Pressable
              ...
              <Text
                style={[styles.seeMoreText, {color: t.palette.primary_500}]}>
                <Trans>Ver todos los memes ({filteredMemes.length})</Trans>
              </Text>
            </Pressable>
          )}
        </View>
          ...
          {filteredDocuments.length > 6 && (
            <Pressable
              ...
              <Text
                style={[styles.seeMoreText, {color: t.palette.primary_500}]}>
                <Trans>
                  Ver todos los documentos ({filteredDocuments.length})
                </Trans>
              </Text>
            </Pressable>
          )}
